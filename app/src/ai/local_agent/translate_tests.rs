...
fn translator() -> Translator {
    Translator::new("task-1".to_owned(), true, "req-1".to_owned())
}

fn continuing_translator() -> Translator {
    Translator::new("task-1".to_owned(), false, "req-1".to_owned())
}
...
fn a_new_conversation_is_told_about_its_task() {
    ...
    assert_eq!(events.len(), 2, "expected StreamInit then CreateTask");
    ...
    assert!(
        create.task.as_ref().unwrap().messages.is_empty(),
...
fn a_conversation_that_already_has_a_task_is_not_told_again() {
    ...
    assert_eq!(events.len(), 1, "only StreamInit: {events:?}");
}
...
fn the_task_is_announced_once_even_if_claude_reinitializes() {
    ...
    assert_eq!(second.len(), 1, "a second CreateTask would be rejected");
}
