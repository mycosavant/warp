...
| diff panel | **inside the distribution** (`RemoteDiffStateModel` over `GetDiffState`). Opened on this repo: header names the distribution, a clean tree says "No open changes", two changes made outside the pane appeared with their hunks within 8 s. ~~**Its commit dialog's generated message does not arrive routed**~~ — true from 2026-09-07 to **2026-09-12**, when the protocol addition the run record asked for was built. The daemon generated the message with its own `AIClient` and answered *"No AI endpoint is configured"*, because the fork's endpoint, model and key live in the GUI's `settings.toml` and OS keychain and the daemon is inside the distribution. `GitGenerateCommitMessageRequest` now carries `return_diff_only`: the client sets it under fork policy, the daemon returns the diff it computed beside the files, and the GUI calls the model. **Re-measured live the same day** (`.fork/runs/routedcommit-2026-09-12/`): routed, the dialog fills in, one model request at the moment it opens, and the message names a function added minutes earlier so the diff provably travelled. The unrouted case was re-run as a control, because the fix refactored a function the local path shares. **The two PR-content paths had the same defect and were fixed the same day** (`GitCreatePrRequest.title`/`body`/`return_inputs_only`, `GitCommitChainRequest.return_pr_inputs`): the daemon returns the diff and the branch's commit subjects, the client generates, the daemon runs `gh pr create` with them. Two round trips rather than one, because the PR diff is taken against `origin/<branch>` and so cannot be computed until the chain's own commit and push have landed. ~~**Not driven live** — that would mean opening a real pull request~~ **Driven live 2026-09-12** (`a5e50790a`), both paths, against a bare on-disk `origin` so `gh` refused before opening a socket: the model requests fired on the GUI side and both refusals named the GitHub host rather than a missing title | 9p, and on this repo it **works**: panel 3 s after the `cd`, the full index (6505 files) 27 s. The "never finishes" this row carried was not reproduced. The no-repository fallback, reached only in a directory that is not one, names the 9p read since `099b26ea5` | measured 2026-09-05, both columns, below |
