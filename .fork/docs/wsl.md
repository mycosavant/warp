...
| diff panel | **inside the distribution** (`RemoteDiffStateModel` over `GetDiffState`). Opened on this repo: header names the distribution, a clean tree says "No open changes", two changes made outside the pane appeared with their hunks within 8 s. ~~**Its commit dialog's generated message does not arrive routed**~~ — true from 2026-09-07 to **2026-09-12**, when the protocol addition the run record asked for was built. The daemon generated the message with its own `AIClient` and answered *"No AI endpoint is configured"*, because the fork's endpoint, model and key live in the GUI's `settings.toml` and OS keychain and the daemon is inside the distribution. `GitGenerateCommitMessageRequest` now carries `return_diff_only`: the client sets it under fork policy, the daemon returns the diff it computed beside the files, and the GUI calls the model. `.fork/runs/localmodel-2026-09-07/` for the measurement; **not re-measured live** — the code is unit-tested and the routed dialog has not been opened since | 9p, and on this repo it **works**: panel 3 s after the `cd`, the full index (6505 files) 27 s. The "never finishes" this row carried was not reproduced. The no-repository fallback, reached only in a directory that is not one, names the 9p read since `099b26ea5` | measured 2026-09-05, both columns, below |
