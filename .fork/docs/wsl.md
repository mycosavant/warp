...
| diff panel | **inside the distribution** (`RemoteDiffStateModel` over `GetDiffState`). Opened on this repo: header names the distribution, a clean tree says "No open changes", two changes made outside the pane appeared with their hunks within 8 s. **Its commit dialog's generated message does not arrive routed** (measured 2026-09-07): the daemon generates it with its own `AIClient`, where the fork's local AI config is not installed; unrouted it fills in. `.fork/runs/localmodel-2026-09-07/` | 9p, and on this repo it **works**: panel 3 s after the `cd`, the full index (6505 files) 27 s. The "never finishes" this row carried was not reproduced. The no-repository fallback, reached only in a directory that is not one, names the 9p read since `099b26ea5` | measured 2026-09-05, both columns, below |
