...
invariants, and where fork behaviour lives. `.fork/docs/manual.md` (this one) is how to
*use* what exists. `.fork/tickets/` holds the tickets and the "as built" record of how
each item actually went. `.fork/archive/SPEC.md` is the original de-telemetry/de-account
reasoning. The `I` files in `.fork/tickets/` are the holding pen in front of the board.
`.fork/archive/CONSOLIDATION.md` is why this fork is the product rather than one of three,
...
"Nothing escapes: measured, not argued" in `.fork/tickets/open-questions.md`.
...
that is the entire point of the kill-switch design in `.fork/archive/SPEC.md`.
...
What you get, scoped by running it (`.fork/tickets/` T6.1):
...
is the faster answer — and it sidesteps the WSL-boundary bugs in `.fork/tickets/`
...
`C:`.** That is `.fork/tickets/` T6.4, and it is a real decision rather than a
...
agent-trajectory feed today. See `.fork/tickets/` I17 for where the trajectory
...
builds. The fork opens it (see `.fork/tickets/` T1) and ports it to Windows.
...
an account. Tracked as `.fork/tickets/` T6.7.
...
This corrects a claim that stood in `.fork/tickets/` T14: **Warp is not blind to an agent's
...
than pending — see `.fork/tickets/` T8.5. The flex of each pane is already
...
"it is not Warp, and X11 is exhausted" in `.fork/tickets/` T8. `window visor toggle`
...
`.fork/tickets/` under T9.1; the Windows half is T9.2.
...
**not on this path at all**. See `.fork/tickets/` T5.1.
...
in `.fork/tickets/` has the timeline.
...
shows up as an absent file. See `.fork/tickets/` T4.7.
...
`.fork/tickets/` I16 has the reasoning; this is the runbook.
