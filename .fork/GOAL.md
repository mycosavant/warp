...
|---|---|---|
| 1 | Merge upstream (T10) | **done**, `b175101a2`; drift check in `.fork/tools/drift-check.sh`, **cron line installed and verified 2026-09-10** — and the recipe needed an added `mkdir -p`, because a redirection opens before the script that creates its own state directory runs |
...
| 5 | Hide the two cloud chips in the agent footer (T19) | **done** 2026-09-06: `/remote-control` is the fork's own (`.fork/docs/remote-control.md`, 2026-09-05); "Hand off to cloud" off by `HandoffLocalCloud` in `FORCE_DISABLED`, which also takes the `&` prefix and `/move-to-cloud` with it |
