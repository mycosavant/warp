...
> **Retired 2026-09-04's horizon, 2026-09-12, by the maintainer's own call: met.**
> All three destination conditions hold — the seven-day product-profile run, the
> friction log (read its own note at 2026-09-12 #4 before concluding from its
> sparseness alone), and all seven board items done or struck. `.fork/next.html`
> is the live plan; this file no longer outranks anything. Its one recorded
> decision moved to
> `.fork/decisions/2026-09-04-acp-stays-the-transport-local-agent-stays-beside-it.md`.
> Kept here per `.fork/GOAL.md`'s own instruction not to destroy a met horizon's
> reasoning — read below for the account, not for current status.
