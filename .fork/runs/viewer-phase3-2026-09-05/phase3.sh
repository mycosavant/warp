...
log "launching PRODUCT + CONSOLE"
# Detached: Warp inherits the redirect, so a foreground launch never returns
# (measured twice now). The discovery record is the readiness signal.
powershell.exe -NoProfile -File 'C:\dev\warp\.fork\tools\warpdev.ps1' -Console -Exe "$WEXE" > "$OUT/launch.txt" 2>&1 &
for i in $(seq 1 30); do sleep 2; CTL instance list | grep -q inst_ && break; done
CTL instance list | tee "$OUT/instance.json" >/dev/null
sleep 6
log "cd the pane"
